...
namespace NinjaTrader.NinjaScript.BarsTypes
{
	public class VolumeBarsType : BarsType
	{
		...
		protected override void OnDataPoint(Bars bars, double open, double high, double low, double close, DateTime time, long volume, bool isBar, double bid, double ask)
		{
			...
			if (bars.Count == 0)
			{
				while (volume > bars.BarsPeriod.Value)
				{
					AddBar(bars, open, high, low, close, time, bars.BarsPeriod.Value);
					volume -= bars.BarsPeriod.Value;
				}
				if (volume > 0)
			...
			else
			{
				...
				{
					volumeTmp = Math.Min(bars.BarsPeriod.Value - bars.GetVolume(bars.Count - 1), volume);
					if (volumeTmp > 0)
				...
				while (volumeTmp > 0)
				{
					AddBar(bars, open, high, low, close, time, Math.Min(volumeTmp, bars.BarsPeriod.Value));
					volumeTmp -= bars.BarsPeriod.Value;
				}
			}
		}
